...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * currentBoard = malloc(sizeof(*currentBoard));
  currentBoard->width = w;
  currentBoard->height = h;
  currentBoard->totalMines = numMines;

  int ** board = malloc(h * sizeof(*board));
  if (board == NULL) {
    fprintf(stderr, "Can not alloc the given size of heap!\n");
    exit(EXIT_FAILURE);
  }
...
int checkBoundary(board_t * b, int x, int y) {
...
int countMines(board_t * b, int x, int y) {
  int number = 0;
  if (x < 0 || x >= b->width || y < 0 || y >= b->height) {
    fprintf(stderr, "Given index of (y,x) is out of the bounds of the board!\n");
    exit(EXIT_FAILURE);
  }

  //Left-up

  if (checkBoundary(b, x - 1, y - 1) && IS_MINE(b->board[y - 1][x - 1])) {
    number++;
  }

  //Up
  if (checkBoundary(b, x, y - 1) && IS_MINE(b->board[y - 1][x])) {
    number++;
  }
  ...
  //Left
  if (checkBoundary(b, x - 1, y) && IS_MINE(b->board[y][x - 1])) {
    number++;
  }
  //Right
  if (checkBoundary(b, x + 1, y) && IS_MINE(b->board[y][x + 1])) {
